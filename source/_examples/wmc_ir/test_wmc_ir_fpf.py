...
### Crawl the folder
img_file = wmcimg.read_folder_list_images(folder, extNoPoint = "FPF", prefix = "", max_recursive_level = 0, data = False)
...
for x in img_file:
    ...
    ### Open FPF image, into object img_fpf
    img_fpf = wmcimg.ir.read_fpf(x.path)
    
    ### See mean
    print "Mean 1 ", np.mean(img_fpf.image)
